<template>
  <div>
    ...
    />
    <!--在线查看xml-->
    <el-dialog :title="xmlTitle" :visible.sync="xmlOpen" width="60%" append-to-body v-drag>
      <div>
        <pre v-highlight>
           <code class="xml">
                {{xmlContent}}
...
export default {
  ...
  methods: {
    ...
    /** 保存xml */
    save(data) {
      //console.log(typeof data.process.id);
      if(!data.process.name) {
        return  Message.error('请输入流程名称');
      }
      if(!data.process.id || data.process.id === ' ') {
        return  Message.error('请输入流程标识');
      }
      if(!data.process.category) {
        return  Message.error('请选择流程分类');
      }
      var params = "_fk=u"
        + (data.process.name ? "&name=" + data.process.name : "")
        + (data.process.category ? "&category=" + data.process.category : "")
      ...
      if (data) {
        if (data.dataType === 'dynamic') {
          if (data.userType === 'assignee') {
            ...
            this.groups = [
              {roleName: "#{approval}", roleId: "#{approval}"}
            ];
          }
        } else {
          this.getDataList();
