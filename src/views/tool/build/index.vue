<template>
  <div class="container">
    ...
    <div class="center-board">
      <div class="action-bar" :style="{'text-align': 'left'}">
        ...
        <el-dropdown trigger="click">
          <el-button icon="el-icon-more" type="text" circle>更多</el-button>
    ...
    <code-type-dialog :visible.sync="dialogVisible" title="选择生成类型" :show-file-name="showFileName"
            @confirm="generate" v-drag />
    <input id="copyNode" type="hidden">

    <!--表单配置详情-->
    <el-dialog :title="formTitle" :visible.sync="formOpen" width="500px" append-to-body v-drag>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="表单类型">
          <el-select v-model="form.formType"  placeholder="请选择">
            <el-option
                v-for="dict in formTypes"
                :key="dict.dictValue"
        ...
        <el-form-item label="表单名称" prop="formName" :rules="[{required: true, message: '请输入表单名称', trigger: 'blur'}]">
          <el-input v-model="form.formName" clearable placeholder="表单名称" style="width: 95%" />
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input v-model="form.remark" clearable placeholder="备注" style="width: 95%" />
        </el-form-item>
      </el-form>
...
import {getForm, createForm, updateForm} from "@/api/flowable/form";
...
export default {
  ...
  watch: {
    ...
    'activeData.__config__.label': function (val, oldVal) {
      ...
      const placeholder = this.activeData.placeholder.replace(oldVal, '');
      const newPlaceholder = placeholder.replace(val, '');
      this.activeData.placeholder = newPlaceholder + val;
    },

    'activeData.options': function (val, oldVal) {
  ...
  methods: {
    ...
    fetchData(component) {
      const { dataType, method, url, remote} = component.__config__
      if (dataType === 'dynamic' && method && url &&!remote) {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.formId != null) {
            ...
            createForm(this.form).then(resp => {
